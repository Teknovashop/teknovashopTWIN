...
type Prefs={name?:string;email?:string;category:string;budget:number;style:string;goal:string}
type Store={prefs:Prefs;setPrefs:(p:Partial<Prefs>)=>void}
const useStore=create<Store>((set)=>({prefs:{category:'fitness',budget:100,style:'minimalista',goal:'mejor calidad/precio'},setPrefs:(p)=>set((s)=>({prefs:{...s.prefs,...p}}))}))
export default function Questionnaire({onResults}:{onResults:(data:any)=>void}){
  ...
  return(<div className="rounded-2xl bg-white shadow p-4 space-y-4">
    ...
    <button onClick={submit} disabled={loading} className="rounded-2xl px-6 py-3 bg-black text-white text-sm">{loading?'Pensando...':'Crear mi carrito inteligente'}</button>
