export function buildAmazonLink({ title, asin, tag }: { title: string; asin?: string; tag: string }) {
...
export function buildSheinLink({ title, pid }: { title: string; pid: string }) {
  const q = encodeURIComponent(title);
  return `https://shein.top/${encodeURIComponent(pid)}?url=${encodeURIComponent(`https://es.shein.com/search/${q}.html`)}`;
}
