export async function generateCopy(input:{prefsSummary:string,productSummary:string}):Promise<string>{
  ...
  const prompt=`Eres un copywriter de e-commerce. Texto breve (70-120 palabras) en español de España.
PREFERENCIAS: ${input.prefsSummary}
PRODUCTOS: ${input.productSummary}`
  try{
    if(!apiKey && !baseUrl){ return 'Selección optimizada para tu estilo y presupuesto.' }
