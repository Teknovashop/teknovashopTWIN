...
export async function POST(req:NextRequest){
  try{
    ...
    const prefs={name:b.name||'',email:b.email||'',category:b.category||'fitness',budget:Number(b.budget)||100,style:b.style||'minimalista',goal:b.goal||'mejor calidad/precio'}
    const products=await allProducts(); const ranked=rankProductsWithScore(products,prefs,3); const top=ranked.map(r=>r.product)
