export type Prefs={category:string;budget:number;style:string;goal:string;audience?:string}
...
export function scoreProduct(p:ProductLite,prefs:Prefs){
  ...
  return 0.22*cat+0.22*budget+0.18*style+0.1*goal+0.1*rating+0.08*popularity+0.1*audience
}
export function rankProductsWithScore<T extends ProductLite>(products:T[],prefs:Prefs,topN=5){return products.map(p=>({product:p,score:scoreProduct(p,prefs)})).sort((a,b)=>b.score-a.score).slice(0,topN)}
