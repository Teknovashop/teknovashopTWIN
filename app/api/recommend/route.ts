...
function score(p: Product, objective: string) {
  // Simple objective-based scoring
  switch (objective) {
    case 'calidad':
      return (p.rating || 3) * 10 - p.price * 0.01
    case 'barato':
      return -p.price + (p.rating || 3)
    case 'mejor_cp':
    default:
      return (p.rating || 3) * 2 - Math.log(Math.max(p.price, 1))
  }
}
...
export async function POST(req: Request): Promise<Response> {
  ...
  const style = (body.style || '').toLowerCase();
  ...
  // attach an image endpoint if missing or generic
  const mapped = items.map(p => {
    const hasReal = p.image && p.image.startsWith('http') && !p.image.includes('picsum.photos');
    const image = hasReal ? `/api/img?src=${encodeURIComponent(p.image)}`
