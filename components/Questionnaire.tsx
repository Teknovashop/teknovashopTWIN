...
export default function Questionnaire({onResults}:{onResults:(data:any)=>void}){
  ...
  return(<div className='rounded-2xl bg-white shadow p-4 space-y-4'>
    <div className='grid sm:grid-cols-2 gap-4'>
      ...
      <div><label className='text-sm'>Estilo</label><select className='w-full rounded-2xl border px-3 py-2' value={prefs.style} onChange={e=>setPrefs({style:e.target.value})}><option value='minimalista'>Minimalista</option><option value='premium'>Premium</option><option value='lowcost'>Low-cost</option><option value='ecologico'>Ecológico</option><option value='alto_rendimiento'>Alto rendimiento</option><option value='retro'>Retro</option><option value='juguetes'>Juguetes</option><option value='deportivo'>Deportivo</option><option value='lujo'>Lujo</option><option value='outdoor'>Outdoor</option><option value='smart-home'>Smart Home</option></select></div>
      ...
      <div><label className='text-sm'>Audiencia</label><select className='w-full rounded-2xl border px-3 py-2' value={prefs.audience} onChange={e=>setPrefs({audience:e.target.value})}><option value='unisex'>Unisex</option><option value='hombre'>Hombre</option><option value='mujer'>Mujer</option><option value='niños'>Niños</option></select></div>
      <div><label className='text-sm'>¿Cuántos productos quieres ver?</label><input type='range' min={3} max={21} value={prefs.n} onChange={(e)=>setPrefs({n:Number(e.target.value)})} className='w-full'/><div className='text-xs text-gray-600'>Mostrando: <b>{prefs.n}</b></div></div>
    </div>
    <button onClick={submit} className='rounded-2xl px-6 py-3 bg-black text-white text-sm'>Ver recomendaciones</button>
